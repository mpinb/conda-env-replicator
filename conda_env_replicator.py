...
def process_dependencies(
    env_data: Dict,
    package_channels: Dict[str, Tuple[str, str]]
) -> List[str]:
    """
...
Examples:
  ...
  %(prog)s -n myenv -o portable_env.yml
    ...
    parser.add_argument('-y', '--yaml', type=Path,
                       help='Input conda environment YAML file')
    parser.add_argument('-e', '--explicit', type=Path,
                       help='Input conda list --explicit output file')
    
    # Environment-based mode
    parser.add_argument('-n', '--name',
                       help='Name of existing conda environment to export')
    
    # Output
    parser.add_argument('-o', '--output', type=Path, required=True,
                       help='Output portable YAML file')
